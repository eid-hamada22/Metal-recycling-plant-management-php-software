...
<?php ob_start();include 'includes/session.php'; ?>
    <body id="body-pd">
            ...
            <nav class="nav" id="nevvv">
                <div>
                    ...
                    <div class="nav__list">
                        ...
                        <a href="fuel_entr.php" class="nav__link active">
                            <i class="fa fa-filter" aria-hidden="true" class="nav__icon"></i>
        ...
        <h1 style="margin-left:880px ;  position: absolute;">مدخولات البنزين</h1>
        <script src="https://unpkg.com/ionicons@5.1.2/dist/ionicons.js"></script>
        
        <div style="float: right;position: absolute; font-size: x-large; margin-top: 50px;"><button type="button" class="btn btn-primary btn-lg" data-bs-toggle="modal" data-bs-target="#name">اضافة عملية ادخال جديدة</button></div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        <input type="number" dir="rtl" placeholder="عدد اللترات" required class="form-control" name="quantity"  style="float: right;">
                                    </div>
                ...
                <div style="float: right;position: absolute; margin-left:1100px ; margin-top: 100px; font-size: x-large">:اخر العمليات</div>
                <table class="table table-hover" style="width: 100%; margin-top: 150px;">
                    <thead>
                        <tr>
                          <th style="width: 28%;"><center>ملاحظات</center></th>
                          <th style="width: 27%;"><center>التاريخ</center></th>
                          <th style="width: 25%;"><center> الكمية (لتر)</center></th>
                    ...
                    <?php
                              include 'includes/conn.php';
                                $sql = "SELECT * FROM fuel_entr ORDER BY id DESC";
                                $result = mysqli_query($conn, $sql);
                                if(mysqli_num_rows($result)>0){
                                    while ($row=mysqli_fetch_assoc($result)) {
                                        echo '
                                    	<tr dir="rtl"> 
                                        <td style=" text-align:center;width: 28%;">'.$row['notes'].'</td>
                                        <td style=" text-align:center; width: 27%;">'.$row['date'].'</td>
                                        <td style=" text-align:center; width: 25%;">'.$row['quantity'].'</td>
...
if (isset($_POST['add'])) {
    ...
    $sql ="INSERT INTO `fuel_entr`(`date`, `quantity`, `notes`) VALUES ('$datea','$quantity','$notes')";

    if ($conn->query($sql)) { 

        header('location:fuel_entr.php ');
    }
    }
