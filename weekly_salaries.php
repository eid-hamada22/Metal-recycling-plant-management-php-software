...
<?php ob_start();     include 'includes/session.php'; ?>
    <body id="body-pd">
        ...
        <h1 style="margin-left:900px ;  position: absolute;">الرواتب الاسبوعية</h1>
        <script src="https://unpkg.com/ionicons@5.1.2/dist/ionicons.js"></script>
        
        <div style="float: right;position: absolute; font-size: x-large; margin-top: 50px;"><button type="button" class="btn btn-primary btn-lg" data-bs-toggle="modal" data-bs-target="#name">اضافة راتب جديد</button></div>
                    ...
                    <form method="post">
                        <div class="modal-dialog ">
                            <div class="modal-content">
                                ...
                                <div style="float: right;">: تاريخ الاعطاء (الخميس)</div>
                                    <div class="form-group">
                                        <input type="date" dir="rtl"   required class="form-control" name="thursday_date" style="float: right;">
                                    </div>
                ...
                <table class="table table-hover" style="width: 100%; margin-top: 150px;">
                    <thead>
                        <tr>
                          <th style="width: 40%;"><center>ملاحظات</center></th>
                          <th style="width: 40%;"><center>تاريخ الاعطاء</center></th>
                    ...
                    <?php
                              include 'includes/conn.php';
                                $sql = "SELECT * FROM weekly_salaries ORDER BY id DESC";
                                $result = mysqli_query($conn, $sql);
                                if(mysqli_num_rows($result)>0){
                                    while ($row=mysqli_fetch_assoc($result)) {
                                        echo '
                                    	<tr dir="rtl" onclick="window.location='."'".'weekly_employee_salaries.php?id='.$row['id'].''."'".';"> 
                                        <td style=" text-align:center; width: 40%;">'.$row['notes'].'</td>
                                        <td style=" text-align:center; width: 40%;">'.$row['thursday_date'].'</td>
...
if (isset($_POST['add'])) {
    ...
    $thursday_date1 = $_POST['thursday_date'];
    $total = $_POST['total'];
    $employees_nu = $_POST['employees_nu'];
    $day_nu = $_POST['day_nu'];
    $thursday_date = date("Y-m-d H:i:s", strtotime($thursday_date1));
    $notes = $_POST['notes'];
    $average = ($total / $employees_nu);
    $sql ="INSERT INTO `weekly_salaries`(`thursday_date`,`notes`) VALUES ('$thursday_date','$notes')";
