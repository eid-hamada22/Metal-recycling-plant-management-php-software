...
<?php ob_start();    include 'includes/session.php'; ?>
    <body id="body-pd">
            ...
            <nav class="nav" id="nevvv">
                <div>
                    ...
                    <div class="nav__list">
                        ...
                        <a href="accountes.php" class="nav__link active">
        ...
        <h1 style="margin-left:880px ;  position: absolute;">الحسابات</h1>
        <script src="https://unpkg.com/ionicons@5.1.2/dist/ionicons.js"></script>
        
        <div style="float: right;position: absolute; font-size: x-large; margin-top: 50px;"><button type="button" class="btn btn-primary btn-lg" data-bs-toggle="modal" data-bs-target="#name">اضافة حساب جديد</button></div>
                ...
                <div style="float: right;position: absolute; margin-left:1100px ; margin-top: 100px; font-size: x-large">:الحسابات</div>
                <table class="table table-hover" style="width: 100%; margin-top: 150px;">
                    <thead>
                        <tr>
                          <th style="width: 35%;"><center>ملاحظات</center></th>
                          <th style="width: 45%;"><center> الاسم</center></th>
                    ...
                    <?php
                              include 'includes/conn.php';
                                $sql = "SELECT * FROM accountes ORDER BY id DESC";
                                $result = mysqli_query($conn, $sql);
                                if(mysqli_num_rows($result)>0){
                                    while ($row=mysqli_fetch_assoc($result)) {
                                        echo '
                                    	<tr dir="rtl"> 
                                        <td style=" text-align:center; width: 35%;">'.$row['notes'].'</td>
                                        <td style=" text-align:center; width: 45%;">'.$row['name'].'</td>
...
if (isset($_POST['add'])) {
    ...
    $sql ="INSERT INTO `accountes`(`name`, `notes`) VALUES ('$name','$notes')";

    if ($conn->query($sql)) { 

        header('location:accountes.php ');
    }
    }
