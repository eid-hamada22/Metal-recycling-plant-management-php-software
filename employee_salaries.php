...
<?php ob_start(); ?>
    <body id="body-pd">
            ...
            <nav class="nav" id="nevvv">
                <div>
                    ...
                    <div class="nav__list">
                        ...
                        <a href="employee_salaries.php" class="nav__link active">
        ...
        <h1 style="margin-left:900px ;  position: absolute;">رواتب الموظفين</h1>
        <script src="https://unpkg.com/ionicons@5.1.2/dist/ionicons.js"></script>
        
        <div style="float: right;position: absolute; font-size: x-large; margin-top: 50px;"><button type="button" class="btn btn-primary btn-lg" data-bs-toggle="modal" data-bs-target="#name">اضافة راتب موظف جديد</button></div>
                <div class="modal fade" id="name"  tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <form method="post">
                        <div class="modal-dialog ">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">اضافة راتب جديد</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    

                                <div style="float: right;">: رقم الراتب الاسبوعي</div>
                                    <div class="form-group">
                                        <select name="weekly_salaries_id" id="" dir="rtl" placeholder="الراتب الاسبوعي التابع له" required class="form-control"   style="float: right;">
                                    <?php
                                            include 'includes/session.php';

                                            $sql1 = "SELECT * FROM weekly_salaries ORDER BY id DESC";
                                    ...
                                    <div style="float: right;">: اسم الموظف</div>
                                    <div class="form-group">
                                    <select name="employee_nm" id="" dir="rtl" placeholder="الراتب الاسبوعي التابع له" required class="form-control"   style="float: right;">
                                    <?php
                                            $sql1 = "SELECT * FROM muazafun ORDER BY id DESC";
                                            $result1 = mysqli_query($conn, $sql1);
                                            if(mysqli_num_rows($result1)>0){
                                                while ($row1=mysqli_fetch_assoc($result1)) {
                                                    echo '
                                    ...
                                    <div style="float: right;">: يومية الموظف</div>
                                    <div class="form-group">
                                        <input type="number" dir="rtl" placeholder="مرتب الموظف اليومي"  required class="form-control" name="daily_salary" style="float: right;">
                                    </div>


                                    <div style="float: right;">: عدد ايام العمل</div>
                                    <div class="form-group">
                                        <input type="number" dir="rtl" step="0.5" placeholder="عدد ايام العمل "  required class="form-control" name="days" style="float: right;">
                                    </div>
                                    
                                    <div style="float: right;">: السلف السابقة </div>
                                    <div class="form-group">
                                        <input type="number" dir="rtl" step="0.5" placeholder="السلف المأخوذة"  required class="form-control" name="advance" style="float: right;">
                                    </div>

                                    <div style="float: right;">: الراتب الاسبوعي</div>
                                    <div class="form-group">
                                        <input type="number" dir="rtl" step="0.5" placeholder="الراتب الاسبوعي المعطى"  required class="form-control" name="amount" style="float: right;">
                                    </div>
                ...
                <div style="float: right;position: absolute; margin-left:1100px ; margin-top: 100px; font-size: x-large">:الرواتب</div>
                <table class="table table-hover" style="width: 100%; margin-top: 150px;">
                    <thead>
                        <tr>
                          <th style="width: 20%;"><center>ملاحظات</center></th>
                          <th style="width: 20%;"><center>رقم الراتب الاسبوعي المتبوع</center></th>
                          <th style="width: 20%;"><center>الراتب المعطى</center></th>
                          <th style="width: 20%;"><center>السلف المأخوذة</center></th>
                          <th style="width: 10%;"><center>الراتب اليومي</center></th>
                          <th style="width: 20%;"><center>ايام العمل</center></th>
                          <th style="width: 20%;"><center>اسم الموظف</center></th>
                    ...
                    <?php
                              include 'includes/conn.php';
                                $sql = "SELECT * FROM employee_salaries ORDER BY id DESC";
                                ...
                                if(mysqli_num_rows($result)>0){
                                    while ($row=mysqli_fetch_assoc($result)) {
                                        ...
                                        <td style=" text-align:center; width: 20%;">'.$row['weekly_salaries_id'].'</td>
                                        <td style=" text-align:center; width: 20%;">'.$row['amount'].'</td>
                                        <td style=" text-align:center; width: 10%;">'.$row['daily_salary'].'</td>
                                        <td style=" text-align:center; width: 20%;">'.$row['days'].'</td>
                                        <td style=" text-align:center; width: 20%;">'.$row['name'].'</td>
                                        <td style=" text-align:center; width: 10%;">'.$row['id'].'</td>
                                        </tr>';                
                                    }
                                }
                    ?>
...
if (isset($_POST['add'])) {
    ...
    $weekly_salaries_id = $_POST['weekly_salaries_id'];
    $days = $_POST['days'];
    $notes = $_POST['notes'];
    $amount = $_POST['amount'];
    $advance = $_POST['advance'];
    $sql ="INSERT INTO `employee_salaries`(`name`, `days`, `daily_salary`,`advance`,`weekly_salaries_id`, `amount`, `notes`) VALUES ('$employee_nm','$days','$daily_salary','$advance','$weekly_salaries_id','$amount','$notes')";

    #$sql2 ="INSERT INTO `expenses`(`date`, `account`, `amount`, `notes`) VALUES ('$datea','راتب موظف','$amount','$notes , 'راتب الموظف $employee_nm')";

    if ($conn->query($sql)) {

        header('location:employee_salaries.php ');
    }
    }
